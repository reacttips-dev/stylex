...
// Packages to release
let publicPackages = {
  "@ladifire-opensource/babel-plugin-transform-stylex": "beta.4",
  "@ladifire-opensource/stylex": "beta.4",
  "@ladifire-opensource/stylex-theme": "beta.4",
  "@ladifire-opensource/stylex-webpack-plugin": "beta.4",
  "@ladifire-opensource/stylex-nextjs-plugin": "beta.4",
};
...
let infoStr = exec("yarn --json workspaces info").toString();

let info = JSON.parse(JSON.parse(infoStr).data);
...
async function getExistingPackages() {
  ...
  for (let [name, { location }] of releasedPackages) {
    promises.push(
      fetch(`https://registry.npmjs.com/${name}`, { method: "HEAD" }).then(
        (res) => {
          if (res.ok) {
            existing.add(name);
          }
...
async function promptVersions(versions) {
  for (let [name, [oldVersion, newVersion, private]] of versions) {
    if (newVersion !== oldVersion || private) {
      console.log(
        `${name}: ${chalk.blue(oldVersion)}${
          private ? chalk.red(" (private)") : ""
  ...
  return new Promise((resolve, reject) => {
    rl.question("Do you want to continue? (y/n) ", function (c) {
      rl.close();
      if (c.replace(/\s+/g, "").toLocaleLowerCase() === "n") {
        reject("Not continuing");
      } else if (c.replace(/\s+/g, "").toLocaleLowerCase() === "y") {
        resolve();
      } else {
    ...
    for (let dep in pkg.dependencies) {
      if (versions.has(dep)) {
        ...
        pkg.dependencies[dep] =
          (status === "released" ? "^" : "") + versions.get(dep)[1];
      }
    }
